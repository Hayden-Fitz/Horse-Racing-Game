...
assert.equal(setup.normalize({ sabotage: "false" }).sabotage, false);
assert.equal(setup.normalize({ sabotage: false }).sabotage, false);
assert.equal(setup.normalize({ sabotage: "invalid" }).sabotage, true);
setup.apply({ sabotage: false });
assert.equal(HD.CONFIG.sabotageEnabled, false);
setup.resetForOnline();
