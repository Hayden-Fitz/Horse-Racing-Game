...
assert.equal(setup.normalize({ sabotage: "false" }).sabotage, undefined);
assert.equal(setup.normalize({ sabotage: false }).sabotage, undefined);
assert.equal(setup.normalize({ sabotage: "invalid" }).sabotage, undefined);
setup.apply({ sabotage: false });
assert.equal(HD.CONFIG.sabotageEnabled, true);
setup.resetForOnline();
assert.equal(HD.CONFIG.sabotageEnabled, true, "Practice rules must not leak into online matches");
