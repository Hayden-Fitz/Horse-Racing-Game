...
var STALE_PLAYER_MS = 3e4;
var LOBBY_CODE = /^[A-HJ-NP-Z2-9]{6}$/;
var worker_default = {
...
var LobbyServer = class {
  ...
  async fetch(request) {
    ...
    try {
      this.maintainState();
      const body = ["PUT", "PATCH", "POST", "RESERVE"].includes(request.method) ? await request.json() : void 0;
  ...
  withinRateLimit(client) {
    const now = Date.now();
    if (now - client.windowStartedAt >= 1e3) {
  ...
  mutate(rawPath, rawMethod = "GET", body, actor) {
    ...
    if (method === "PATCH") {
      ...
      const next = isRecord(current) && isRecord(body) ? { ...current, ...body } : body;
  ...
  authorize(path, method, body, actor) {
    ...
    if (parts.length === 2 && method === "PUT" && !lobby) {
      const valid = body?.meta?.hostId === actor && body?.players?.[actor]?.id === actor && String(body?.seats?.[0]) === actor && Number(body?.meta?.capacity) === 8;
    ...
    if (parts[2] === "seats") {
      if (method === "RESERVE") {
        ...
        if (!Number.isInteger(seat) || seat < 0 || seat > 7 || seated || full || lobby.meta?.started) {
    ...
    if (parts[2] === "events") {
      if (method === "POST" && isMember && body?.from === actor) return;
      if (method === "DELETE" && isHost) return;
  ...
  broadcast(changedPath, eventType, data) {
    for (const [socket, client] of this.clients) {
      for (const subscribedPath of client.subscriptions) {
        if (changedPath !== subscribedPath && !changedPath.startsWith(`${subscribedPath}/`)) continue;
        const relative = changedPath === subscribedPath ? "/" : `/${changedPath.slice(subscribedPath.length + 1)}`;
        this.send(socket, {
          type: "update",
  ...
  maintainState() {
    ...
    for (const [code, lobby] of Object.entries(this.state.lobbies)) {
      ...
      if (!players.length && now - Number(lobby.meta?.updatedAt || 0) > 6e4) {
      ...
      for (const [eventId, event] of Object.entries(lobby.events || {})) {
        if (now - Number(event.createdAt || 0) > 3e4) delete lobby.events[eventId];
      }
    }
  }
  getAt(path) {
    ...
    for (const part of path.split("/")) {
      if (!isRecord(current) || !Object.hasOwn(current, part)) return void 0;
...
__name(cleanClientId, "cleanClientId");
...
__name(normalizeRuleWrite, "normalizeRuleWrite");
function normalizeRules(input = {}) {
  const integer = /* @__PURE__ */ __name((value, minimum, maximum, fallback) => {
    const number = Number(value);
    return Number.isFinite(number) ? Math.min(maximum, Math.max(minimum, Math.round(number))) : fallback;
  }, "integer");
  const crowds = /* @__PURE__ */ new Set(["off", "relaxed", "normal", "lively"]);
  return {
    ...
    startingMoney: integer(input.startingMoney, 100, 1e3, 100),
...
function corsResponse(value, status) {
  return withCors(value === null ? new Response(null, { status }) : jsonResponse(value, status));
}
